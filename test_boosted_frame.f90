...
module ModTestBoostedFrame
  ...
  use ModExactRs
  use ModPlotFile
  use ModBoostedFrame, ONLY: set_param, update_hydro_var, update_mhd_var,&
       fast_mhd_speed
  implicit none
...
contains
  ...
  subroutine test_hydro_x
    ...
    call save_plot_file(NameFile='test_hydro_x.out', &
  ...
  subroutine test_mhd_x
    ...
    ! Init boosted frame
    !--------------------------------------------------------------------------
    call set_param(tOffsetPerRin=tOffsetPerR, GammaIn = Gamma)
    ...
    do
       ...
       do i = nCell/2+1, nCell
          call update_mhd_var(Old_V=StateOld_VC(:,i),&
               Source_V=0.5*Dt/Ds*(Flux_VF(:,i-1) - Flux_VF(:,i)),&
               Radial_D=[1.0, 0.0, 0.0],&
               New_V=State_VC(:,i))
       end do
    ...
    subroutine get_time_step
      ...
      do iCell = nCell/2 + 1, nCell
         Dt_C(iCell) = Dt_C(iCell)*(1 - tOffsetPerR*&
              ...
              fast_mhd_speed(State_VC(:,iCell),Normal_D)))
      end do
      Dt = minval(Dt_C)
    end subroutine get_time_step
    !==========================================================================
...
program test_program
  ...
  use ModBoostedFrame
  implicit none
  real :: Old_V(9), Source_V(9), Radial_D(3), New_V(9)
  real :: Old8_V(8), Source8_V(8), New8_V(8)
  !----------------------------------------------------------------------------
  call set_param(tOffsetPerRin=1/10.7651851771994416)
  Old_V = [4.3705443170149918E-07, 0.5374243877957461, 1.7761451948874185,&
       -0.4816919427997621,1.6066075245824965E-04, 5.2845875105385516E-04,&
       -1.4381367134130662E-04,5.6708280207653453E-08,3.6760644038367720E-08]
  Source_V=[1.3123383717066699E-06,1.4407959045112049E-06,8.9594698428502407E-06,&
       -3.0038351622945924E-06,5.2301753269067377E-04,-4.7698198367723245E-05,&
       1.8350336652742729E-04,1.8838631025831460E-09,4.1411512583313943E-05]
  Radial_D= [0.1627027264722773, 0.9529731121947672,  -0.2556757130278643]
  New_V= [4.3612702658706328E-07,0.5375524274926966,1.7744813289702237,&
       -0.4812764245762391, 1.6036696447860609E-04,5.2998125228292329E-04,&
       -1.4415352529177479E-04, 5.6709857960503877E-08, 3.6688267042638970E-08]
  write(*,*) 'Start unit test MHD'
  Old8_V(1:7) = Old_V(1:7); Old8_V(8) = Old_V(9)
  Source8_V(1:7) = Source_V(1:7); Source8_V(8) = Source_V(9)
  New8_V(1:7) = New_V(1:7); New8_V(8) = New_V(9)
  call update_mhd_var(Old8_V, 0.5*Source8_V, Radial_D, New8_V)
  write(*,*) 'Start unit test MHD+Pe'
  call update_mhd_pe_var(Old_V, 0.5*Source_V, Radial_D, New_V)
